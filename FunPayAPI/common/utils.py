...
def parse_funpay_datetime(date_text: str) -> datetime:
    """Парсит дату/время из текста FunPay."""
    now = datetime.now(tz=timezone(timedelta(hours=3)))
    if any(today in date_text for today in ("сегодня", "сьогодні", "today")):
        h, m = date_text.split(", ")[1].split(":")
        return datetime(now.year, now.month, now.day, int(h), int(m))
    elif any(yesterday in date_text for yesterday in ("вчера", "вчора", "yesterday")):
        h, m = date_text.split(", ")[1].split(":")
